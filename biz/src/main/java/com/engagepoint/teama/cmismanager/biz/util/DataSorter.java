...
public final class DataSorter {
     ...
     * @see com.engagepoint.teama.cmismanager.biz.ejb.ConvertorEJB
    ...
    public static void validateTypeDefinition(TypeDefinition type) throws ValidationException {
        ...
        if(type.getTypeMutability() == null) {
            throw new ValidationException("TypeMutability must be set.");
        } else {
            if (type.getTypeMutability().canCreate() == null
                    || type.getTypeMutability().canUpdate() == null
                    || type.getTypeMutability().canDelete() == null) {
                throw new ValidationException("TypeMutability CRUD must be set.");
            }
        }
